...
clean_block <- function(filename) {
  ...
  if (n_trials < 400) {
    warning(glue('Skipping {basename(filename)}, only {n_trials} trials...'))
    return(NULL)
  }
  ...
  raw_dat$trials$probe_px <- NULL # TODO: store probe location
  
  block_data <- as.data.table(raw_dat$trials)
  block_data[, `:=`(id = raw_dat$block$id,
                    group = raw_dat$block$group,
                    trial = 1:.N,
                    clamp_side = ifelse(clamp_angle==300, 'right', 'left'),
                    trial_start_time = raw_dat$trials$frames[[1]]$disp_time[1])] # TODO: think about this more
  block_data
}
...
compute_reach_angle <- function(reach_data, state_names, origin, target) {
  ...
  while (i <= nrow(foo) - 1) {
    if (!is.na(foo[i+1]$t) && !is.na(foo[i]$t) && foo[i+1]$t - foo[i]$t < 0.001) {
      foob[[i]] <- list(t = foo[i]$t, x = foo[i+1]$x, y = foo[i+1]$y, state = foo[i+1]$state)
      i <- i + 2
    } else {
      foob[[i]] <- list(t = foo[i]$t, x = foo[i]$x, y = foo[i]$y, state = foo[i]$state)
      i <- i + 1
    }
  }
